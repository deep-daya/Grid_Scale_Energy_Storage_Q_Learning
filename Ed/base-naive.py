    ...
    for index, row in df.iterrows():
        ...
        if row['tariff'] == min_tariff:
            ...
            if df.loc[index,'bat_eng']  < BAT_KWH_MAX:
                df.loc[index, 'bat_charge'] = min(BAT_KWH_MAX- df.loc[index,'bat_eng'] , HR_FRAC *BAT_KW)
        ...
        #try to discharge as much as possible at max tariff
            ...
            if df.loc[index,'bat_eng'] > BAT_KWH_MIN:
                df.loc[index,'bat_discharge'] = min(df.loc[index,'bat_eng']  - BAT_KWH_MIN, HR_FRAC *BAT_KW)
                df.loc[index,'bat_eng'] -= df.loc[index,'bat_discharge'] 

        #account for battery charge/discharge in the reward
        df.loc[index,'R_naive'] += (row['gridnopv']-df.loc[index,'bat_discharge'] + df.loc[index, 'bat_charge']) * row['tariff']
